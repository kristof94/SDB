...
public class MHTUtil {
	private static enum TransferEncoding {Null, Base64, QuotedPrintable};
	...
	public static TransferEncoding checkTransferEncoding(String transferEncoding) {
		...
		else return TransferEncoding.Null;
	}
	...
	private static class MHT {
		...
		private void addEntity(BufferedReader in, String boundary) throws Exception {
			...
			if(content==null && entity.type!=null && entity.type.contains("text")) {
				...
				entityName = (i2>-1?entity.location.substring(i2+1, i1):"");
				...
				readEntity(in, out, transferEncoding, boundary);
				replaceMap.put(entity.location, dir.getName()+"/"+entityName);
				if(entityName.contains(extension[0]) || entityName.contains(extension[1]) || entityName.contains(extension[2]) || entityName.contains(extension[3])  || entityName.contains(extension[4])  || entityName.contains(extension[5]) || entityName.contains(extension[6]) || entityName.contains(extension[7]) )
					;
				else
					{
